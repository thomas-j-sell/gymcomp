...
static void select_click_handler(ClickRecognizerRef recognizer, void *context) {
  ...
  if(mode) {
    effect_layer_add_effect(edit_layer, effect_invert, NULL);
  }else{
    effect_layer_remove_effect(edit_layer);
  }
  layer_mark_dirty(effect_layer_get_layer(edit_layer));
  // eff
}
...
static void window_load(Window *window) {
  ...
  edit_layer = effect_layer_create((GRect) { .origin = { 0, bounds.size.h/2 }, .size = { bounds.size.w, bounds.size.h/2 } });
  ...
  layer_add_child(window_layer, effect_layer_get_layer(edit_layer));
}

static void window_unload(Window *window) {
  text_layer_destroy(exercise_layer);
  text_layer_destroy(weight_layer);
  effect_layer_destroy(edit_layer);
}
